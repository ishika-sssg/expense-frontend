...
class SettleupBloc extends Bloc<SettleupEvent, SettleupState> {
  ...
  SettleupBloc({required this.authStorage, required this.expenseDetails})
      : super(SettleupInitial()) {
    on<SettleupEvent>((event, emit) {});

    on<GetAllSettlementDetailsEvent>((GetAllSettlementDetailsEvent event,
        Emitter<SettleupState> emit) async {
      ...
      try {
        ...
        if (userId != null) {
          final unsettledTrans =
              await expenseDetails.getAllUnsettledTransactionsApi(
                  event.group_id, res["user_id"].toString());
          // print("from settlement transaction $unsettledTrans");
          if (unsettledTrans["status"] == 200) {
            emit(GetUserDetailsSuccess(
                userDetails: res,
                unsettledTransactions: unsettledTrans["data"],
                expenseSummary: ""));
          }
        } else {
    ...
    on<MakeSettlementEvent>(
        (MakeSettlementEvent event, Emitter<SettleupState> emit) async {
      try {
        final res = await authStorage.retrieveData();
        final userId = res["user_id"];
        ...
        if (userId != null) {
          final settleTrans =
              await expenseDetails.SettleLoggedinUserTransactions(
                  event.trans_id, event.user_id);
