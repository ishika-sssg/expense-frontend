...
class ViewSettlementBloc
    extends Bloc<ViewSettlementEvent, ViewSettlementState> {
  final AuthStorage authStorage;
  final ExpenseDetails expenseDetails;

  ViewSettlementBloc({required this.authStorage, required this.expenseDetails})
      : super(ViewSettlementInitial()) {
    on<ViewSettlementEvent>((event, emit) {});

    on<GetAllSettlementsEvent>((GetAllSettlementsEvent event,
        Emitter<ViewSettlementState> emit) async {
      try {
        final userDetail = await authStorage.retrieveData();

        final res = await expenseDetails.GetAllSettlementRecord(
            userDetail["user_id"].toString());
